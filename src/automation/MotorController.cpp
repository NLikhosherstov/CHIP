...
void MotorController::setStep(int8_t step_0_to_4) {
  const uint8_t speed = (step_0_to_4 < 0) ? 0 : (step_0_to_4 > 4) ? 4 : step_0_to_4;
  const bool enabled = m_state.getMotorState().enabled;
  const uint8_t pwm = pwmForStep(m_config.getConfig(), speed);
  const uint16_t duty = enabled ? static_cast<uint16_t>(pwm) * 4 : 0;
  m_state.setMotorState(enabled, speed, duty, enabled ? 20000 : 0);
}
